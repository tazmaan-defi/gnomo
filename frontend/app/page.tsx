...
export default function Home() {
  ...
  const handleMintPosition = async () => {
    if (!walletAddress || !selectedClmmPool) return
    // For one-sided liquidity, only one amount may be provided
    const hasAmount = (parseFloat(mintAmountA) || 0) > 0 || (parseFloat(mintAmountB) || 0) > 0
    if (!hasAmount) return
    setClmmLoading(true)
    const loadingToast = toast.loading('Creating Position', 'Please confirm in your wallet...')
    try {
      const amtA = BigInt(Math.floor((parseFloat(mintAmountA) || 0) * 1_000_000))
      const amtB = BigInt(Math.floor((parseFloat(mintAmountB) || 0) * 1_000_000))
  ...
  const clmmTvlUsd = allClmmPositions.reduce((sum, pos) => {
    ...
    let amtA = 0, amtB = 0
    if (pC <= pL) {
      amtA = liq * (sqrtPU - sqrtPL) / (sqrtPL * sqrtPU)
    } else if (pC >= pU) {
      amtB = liq * (sqrtPU - sqrtPL)
    } else {
      amtA = liq * (sqrtPU - sqrtPC) / (sqrtPC * sqrtPU)
      amtB = liq * (sqrtPC - sqrtPL)
    }
    const tvl = (amtA + amtB) / 1_000_000 * GNOT_PRICE_USD
    return sum + tvl
  }, 0)
      ...
      <header className="border-b border-[#21262d] bg-[#161b22]">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-8">
            <h1 className="text-xl font-bold text-[#238636]">Gnomo DEX <span className="text-xs font-normal text-[#8b949e]">v0.9.13</span></h1>
                      ...
                      <div className="mt-2 p-2 bg-[#21262d] rounded-lg text-sm space-y-1">
                        {(() => {
                          ...
                          for (const pos of poolPositions) {
                            ...
                            totalA += amtA / 1_000_000
                            totalB += amtB / 1_000_000
                            if (inRange) {
                              activeA += amtA / 1_000_000
                              activeB += amtB / 1_000_000
                            }
                          }
                          const totalTokens = totalA + totalB
                          const activeTokens = activeA + activeB
                          ...
                          return (
                            <>
                              ...
                              <div className="flex justify-between pt-1 border-t border-[#30363d]">
                                <span className="text-[#8b949e]">TVL</span>
                                <span className="font-medium text-[#238636]">~${totalTokens.toFixed(2)}</span>
                              </div>
                              <div className="flex justify-between text-xs">
                                <span className="text-[#8b949e]">Active Liquidity</span>
                                <span className="text-[#58a6ff]">${activeTokens.toFixed(2)}</span>
                              </div>
                            </>
                          )
                        })()}
